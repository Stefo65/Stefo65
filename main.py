...
def send_welcome(message):
    bot.send_message(message.chat.id,
                     "Salve, benvenuto in AIFG. \n Cosa desideri fare ? \n /help per vedere i comandi disponibili")


@bot.message_handler(commands=['help'])
def send_help(message):
    bot.reply_to(message, "Elenco dei comandi:\n /start - per iniziare \n /help - aiuto \n"
                          " /tappa - per vedere le classifiche dell'ultima tappa \n"
                          " /calendario - per vedere il calendario delle gare AIFG")
...
def send_calendario(message):
    bot.send_message(message.chat.id,
                     "https://www.footgolf.it/area-giocatori/calendario-gare/")
...
def echo_all(message):
    bot.reply_to(message, "Comando non previsto") #message.text)
